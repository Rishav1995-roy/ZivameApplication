    ...
    open fun doRequestInternal(liveData: MutableLiveData<ResponseWrapper<R>>) {
        makeRequest().enqueue(object : APICallback<R>(errorHandler) {
            override fun handleResponseData(data: R?, statusCode: StatusCode) {
                liveData.value = ResponseWrapper(data = data, statusCode = statusCode, errorJson = "")
            }

            override fun handleException(ex: GenericException, statusCode: StatusCode) {
                liveData.value = ResponseWrapper(ex, statusCode = statusCode, errorJson = ex.errorJson)
            }
        })
    }
